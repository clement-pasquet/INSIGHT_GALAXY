...
app.use(cors({
    origin: 'localhost:8090' //Mettre le lien du serveur web 
  })); 
...
//Ajout d'une planète
 app.post('/planet', (req, res) => {
    ...
    planeteDao.addPlanete(new Planet(newPlanet))
       .then(isCreated => {
        if (isCreated){
            res.send('\x1b[32mPlanète ajouté avec succès !\x1b[0m')
        }else{
 ...
 app.delete('/planet/:name', (req, res) => {
    ...
    planeteDao.deletePlanetsByName(planetName)
       .then(() => {
          res.send('\x1b[32mPlanète supprimée avec succès ! \x1b[0m');
       })
       .catch(err => {
          console.error(err);
